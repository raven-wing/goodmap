...
class PaginationHelper:
    ...
    def apply_pagination_and_sorting(items, page, per_page, sort_by, sort_order):
        ...
        if sort_by:
            ...
            items.sort(
                key=lambda item: PaginationHelper.get_sort_key(item, sort_by),  # type: ignore[reportUnknownLambdaType]
                reverse=reverse,
            )
...
def google_json_db_get_initial_view(self) -> dict[str, Any]:
    ...
    Returns:
        dict: Opening view (center, zoom, max_zoom). Returns empty dict if not found,
              which resolves to the default view.
    """
...
def mongodb_db_get_initial_view(self) -> dict[str, Any]:
    ...
    Returns:
        dict: Opening view (center, zoom, max_zoom). Returns empty dict if config
              document not found or field missing, which resolves to the default view.
    """
    config_doc = self.db.config.find_one({"_id": "map_config"})
    if config_doc:
